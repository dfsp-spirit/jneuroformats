...
# Deploy to gh-pages
git stash && { git checkout gh-pages 2>/dev/null || git checkout --orphan gh-pages; } && cd .. && git stash && { git checkout gh-pages 2>/dev/null || git checkout --orphan gh-pages; } && cd .. && echo "Switched to gh-pages branch and into repo root. Current directory: $(pwd)"
