...
public class Viridis implements Colormap {
    ...
    public Color get(Float position) {
        ...
        if (position < 0) {
            ...
            if (colors.size() <= 1) {
                ...
                final Color l = Viridis.lerp(colors.get(floor), colors.get(floor + 1), t);
                return l;
            }
        }
    }
     ...
     *
     * @author mahdilamb -- https://github.com/mahdilamb/colormap
     * @license Apache-2.0
     *
     ...
     */
    private static float lerp(final float low, final float high, float amt) {
        amt = clamp(amt, 0f, 1f);
     ...
     * @license Apache-2.0
     */
    private static float[] XYZToRGB(float[] out, float x, float y, float z) {
        final float X = x / 100;
        final float Y = y / 100;
        final float Z = z / 100;
     ...
     * @license Apache-2.0
     */
    private static float[] XYZToLab(float[] out, float x, float y, float z) {
     ...
     */
    private static <T extends Comparable<T>> T clamp(final T val, final T min, final T max) {
     ...
     * @license Apache-2.0
     */
    private static float[] RGBToXYZ(float[] out, float r, float g, float b) {
